...
namespace advisorSystem.lib
{
    public class TeacherHelper
    {
        ...
        public JObject UpdateApply(String tid, String tg_id, int accept)
        {
            sqlHelper = new SQLHelper();
            JObject obj = new JObject();
            String query = " update sa set sa_state="+ accept + ", sa_check_by_type=1, sa_check_by_st_id='"+tid+"'" + 
                            " FROM ntust.student_apply sa" +
                            " WHERE sa_tg_id = '"+ tg_id + "' AND sa_t_id = '"+ tid +"'";
